...
async function callBankFunction(name, payload = {}) {
...
async function openBankConnect({ householdId = "", onSuccess, onError } = {}) {
  const tokenData = await callBankFunction("createBankConnectToken", { householdId });
  ...
  const widget = new PluggyConnect({
    ...
    onSuccess: async itemData => {
      ...
      const result = await callBankFunction("registerBankConnection", { householdId, itemId, connectorName });
      onSuccess?.(result);
    },
...
async function syncBankConnection({ householdId = "", itemId }) {
  return callBankFunction("syncBankConnection", { householdId, itemId });
}
